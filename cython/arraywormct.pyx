...
class Worm(object):
		def __init__(self, lattice):
				...
				#identify the next events:
				self.find_next_events()

		def find_next_events(self):
				self.nextevcs = self.lattice.sites[self.headx].find_event_in_dir(self.headt,self.direction)
				nbs = self.lattice.sites[self.headx].neighbours;
				self.nextevnbs = [self.lattice.sites[nb].find_event_in_dir(self.headt, self.direction)
									for nb in nbs]
		...
		def jump(self, dt, decconsts):	
			
			currentspin = self.nextevcs.spins[self.reversedirection]
		...
		def process_event(self,dt):
			#print("process event")
				...
				evccoord = self.headx
				eventtoprocess = self.nextevcs
				currentspin = eventtoprocess.spins[self.reversedirection]
		...
		def find_time_to_nextevents(self):
			dtnbs = [abs(ev.time - self.headt) for ev in self.nextevnbs]
			dtc = abs(self.nextevcs.time- self.headt)
		...
		def get_decay_constants(self):
			spinnbs = [ev.spins[self.reversedirection] for ev in self.nextevnbs ]
			spinc = self.nextevcs.spins[self.reversedirection]
			# neighbour coordinates are:
